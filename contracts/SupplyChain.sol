...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale(uint _sku){
    require(items[_sku].state == State.ForSale && items[_sku].seller != address(0), "Item not on sale yet!");
    _;
  }
  ...
  modifier received(uint _sku){
    require(items[_sku].state == State.Received, "Item hasn't been received!");
    _;
  }
  ...
  function shipItem(uint sku)
    sold(sku) verifyCaller(items[sku].seller)  public
  {
  ...
  function receiveItem(uint sku)
    shipped(sku) verifyCaller(items[sku].buyer) public
  {
